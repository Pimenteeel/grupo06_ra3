...
//bibliotecas utilizadas para funções e manipulações de processos deste código
#include <sched.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define STACK_SIZE (1024 * 64)//tamanho do stack(pilha) que será utilizado por processos filho
char child_stack[STACK_SIZE];

//função principal para os processos filho após sua criação
...
//função que coleta identificadores unicos (inodes) de cada tipo de namespace para cada PID
//coloca os dados na estrutura "ProcessNamespaces" e passa pelos arquivos de namespace em ordem,
//se nao existir arquivo, atribui-se 0, caso contrario temos o inode encontrado salvo
...
// coleta o inode do namespace de montagem (lógica do mnt),
...
// coleta inode dos outros namespaces(uts, ipc, net, pid, user, cgroup, time)
// se encontrado o inode é salvo, caso ausente, preenche-se 0
// identifica o compartilhamento entre processos com a comparação dos inodes
    sprintf(proc_path, "/proc/%d/ns/uts", pid);//uts
    if (stat(proc_path, &buffer) == -1){
        ns -> uts = 0;
    }
    else{
...
//função responsável pela comparação de dois namespaces diferentes(PID1 e PID2)
//primeiro coleta os inodes dos namespaces de cada processo com "namespaces_por_pid()", funcao anterior
//caso nao encontre, retorna erro 
//imprime os inodes em tabela de ambos os processos, destacando se os compartilham ou nao, facilitando... 
//... visualização de isolamento entre processos
...
// essa função percorre a lista de processos do sistemas do /proc, colentando os inodes dos namespaces de cada PID... 
// por meio da funcao anterior namespace_por_pid. Le tambem o nome do comando executado pelo processo 
// gera um relatório CSV contendo informações de timestamp, PID, nome do comando, e os IDs/inodes de todos os namespaces relevantes
// ajudando na analise de compartilhamento de relacoes de isolamento entre processos e grupo de processos
...
// Mede o overhead(Custo extra de tempo) para a criação de novos namespaces PID em relação ao fork tradicional
// Cria multiplos processos filhos usando fork e mede o tempo total gasto
// Cria multiplos processos filhos usando clone com CLONE_NEWPID (novo PID namespace) e mede o tempo total gasto 
// O loop de clone depende da função auxiliar child_fn como funcao de entrada para os filhos criados 
// Calcula diferença de tempo e média de tempo total gasto por criação e exibe no terminal
