...
//Criar grande de tamanho "size"
void create_large_file(const char* filename, size_t size) {
    printf("Criando arquivo %s (%ld MB)...\n", filename, size / (1024 * 1024));

	int fd = open(filename, O_WRONLY | O_CREAT | O_TRUNC, 0644); //Abre/cria arquivo para escrita
    if (fd == -1) {
    ...
    size_t written = 0;
    while (written < size) {
		ssize_t bytes = write(fd, buffer, BUFFER_SIZE); //Escreve buffer no arquivo
        if (bytes == -1) {
...
//Ler ou processar arquivo grande
